...
export default function ArticleCard({ article }: Props) {
  ...
  //   lower: true,
  // })
  const slug = addDashToSpace(article.title)
  // console.log(slug)
  ...
  return (
    <a href={`/blog/${slug}`}>
      <div className="flex flex-col overflow-hidden cursor-pointer group">
        ...
        <div className="flex flex-col justify-between flex-1 py-4 bg-white dark:bg-neutral-900" >
          <div className="flex-1">
            <p className="text-xl font-semibold text-gray-900 dark:text-white font-anuphan">{article.title}</p>
            <p className="mt-3 text-base text-gray-500 dark:text-gray-300  line-clamp-2">{article.summary}</p>
          </div>
          <div className="flex items-center mt-4">
            <div className="flex mb-2 space-x-1 text-sm text-gray-400">
              {article.categories.map(category => (
                <div key={category}>
                  <span className="font-semibold text-gray-600 dark:text-gray-200">{category} </span>
